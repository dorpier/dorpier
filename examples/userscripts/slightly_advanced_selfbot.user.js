...
(async function () {
    // has to be an async function so we can use the userid-get function
    "use strict";
    client = new Client();
    ...
    client.on("ready", async () => {
        // onready event
        Utils.Logger.log(
            `Logged into ${client.user.username}! Prefix is ${PREFIX}`,
        );
    ...
    client.on("message_create", async (message) => {
        ...
        if (message.content.trim() == `${PREFIX}ping`) {
            // you have to call message.content.trim() when getting message content because if you don't, there's sometimes whitespace that makes your message checking not work.
