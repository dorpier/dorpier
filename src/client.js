...
import patcher from "./webpack/patcher.js";
...
export class Client {
  ...
  on(event, callback, raw = false) {
    if (raw) {
      ...
      return () => modules.dispatcher.unsubscribe(event.toUpperCase(), wrapper);
    }
  }
  ...
  createDM(id) {
    const existing = webpack.getModule("getDMFromUserId").getDMFromUserId(id);
    if (existing) {
