...
class Client {
    constructor() {
        this.token = Discord.get_token();
        this.events = {};
    }

    get user() { // Property is better because it's often overwritten by Discord
        return Discord.find_module.by_props("getCurrentUser").getCurrentUser();
    }
    ...
    create_slash_command(name, description, options = [], callback) {
        Discord._create_command(name, description, options, 1, callback);
    }
    ...
    send_ephemeral_message(
        content = "", embeds = [], author = Client.user, type = 0, tts = false, sticker_ids = []
    ) {
        let msg = Discord._create_message(content, embeds);
        msg.author = author;
        msg.type = type;
        msg.tts = tts;
