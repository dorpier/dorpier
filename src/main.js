...
(Logger = {
    ...
    log(message) {
        if (this._log == true) {
            console.log(
                `%c[discordjs-pure] (LOGGER)%c ${message}`,
                "color: #9e0700",
                "color: #ffffff",
            );
        }
...
Discord = {
    ...
    _createCommand(name, description, options, type, callback) {
        ...
        this.getModule("BUILT_IN_COMMANDS").BUILT_IN_COMMANDS.push({
            ...
            displayName: name,
            id: `-${
                this.getModule("BUILT_IN_COMMANDS").BUILT_IN_COMMANDS.length + 1
            }`,
    ...
    async createAlertModal(title, descr) {
        ...
        let Markdown = () => {
            modules = [];
            filter = (m) =>
                m.default?.displayName === "Markdown" && m.default.rules;
    ...
    getModule(n, f = true) {
        // whether to return the first module
        let x = false;
        let mod;
        window.webpackChunkdiscord_app.push([
            [Math.random()],
...
Client = class Client {
    ...
    sendMessage(
        ...
        Discord.getModule("sendMessage").sendMessage(
            channel,
            msg,
            null,
            params,
    ...
    sendClydeMessage(content) {
        ...
        Discord.getModule("sendBotMessage").sendBotMessage(
            Discord._getCurrentChannelID(),
            content,
        );
    }
    sendClydeError() {
        Discord.getModule("sendBotMessage").sendClydeError(
            Discord._getCurrentChannelID(),
        );
    }
    connect() {
        let state = this;
        Discord.getModule("Dispatcher").Dispatcher.prototype._interceptor =
            function (e) {
