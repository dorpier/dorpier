...
Plugins = {
  ...
  _corePlugins: [{
                ...
                start: function() {
                    ...
                    _originalDiscordSentry = window.DiscordSentry;
...
Utils = {
    Logger: {
        ...
        log(toLog) {
            if (_KEEPLOGS == true) {
                console.log(`%c[discordjs-pure] (LOGGER)%c ${toLog}`, 'color: #9e0700', 'color: #ffffff'); // [discordjs-pure] (LOGGER) is red, the text that's logged is white.
            }
        }
    },
...
Discord = {
    ...
    getToken() { // this function has two fallbacks, so it should always work.
...
Client = class Client {
    ...
    get user() { // Property is better because it's often overwritten by Discord
    ...
    sendMessage(
        channel = null, content = "", tts = false, messageReference = null, allowedMentions = null, stickerIDs = null
    ) {
