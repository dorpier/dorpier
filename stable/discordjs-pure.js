...
client.run = function(GLOBAL_USER_TOKEN, bot) {
    if (bot == true){
    let token = `Bot ${GLOBAL_USER_TOKEN}`;
    ...
    ws.addEventListener("message", function incomming(data) {
      ...
      console.warn("[discordjs-pure] bot accepts only 'true' or 'false'; you said undefined or didn't specify the type! can't log in...");
    }
    else {
      console.warn("[discordjs-pure] bot accepts only 'true' or 'false'; you said undefined or didn't specify the type! can't log in...");
    }
